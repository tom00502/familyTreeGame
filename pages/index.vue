<template>
  <div class="min-h-screen bg-[#FAF8F3] flex flex-col items-center justify-center p-6">
    ...
    <div class="w-full max-w-[390px] flex flex-col items-center gap-8 relative z-10">
      <!-- Logo -->
      <div class="w-24 h-24 bg-[#8B2635] rounded-full flex items-center justify-center border-4 border-[#D4AF37] shadow-lg">
      ...
      <div class="w-full space-y-3">
        ...
        <div class="grid grid-cols-3 gap-3">
          <button
            v-for="duration in [90, 120, 180]"
            :key="duration"
            @click="selectedDuration = duration"
            class="py-3 rounded-md border-2 transition-all font-medium"
            :class="
      ...
      <div
        v-if="error"
        class="w-full p-3 bg-red-100 border border-red-300 rounded-md text-red-700 text-sm"
      >
        {{ error }}
      </div>
      ...
      <div class="w-full text-center">
        <p class="text-[#8B8278] text-sm">
          ...
          <button
            ...
            class="text-[#8B2635] font-medium underline hover:text-[#5C2E2E]"
          >
            直接加入
          </button>
        </p>
...
const { connect, isConnected, createRoom, error: wsError, clearError } = useGameWebSocket()
